...
namespace Gomaji_API
{
    public class WriteLog
    {
        ...
        public WriteLog()
        {
            _sLogPath = ConfigurationManager.AppSettings["WRITE_LOG_PATH"];
            string sWriteLog = ConfigurationManager.AppSettings["WRITE_LOG"];
            _IsWriteLog = ((sWriteLog == "Y") && (_sLogPath.Length > 0)); //除了有開之外, 而且必需指定 Log 路徑
            //
            _sErrLogPath = ConfigurationManager.AppSettings["ERROR_LOG_PATH"];
            string sWeritErrLog = ConfigurationManager.AppSettings["ERROR_LOG"];
        ...
        {
            string vLogFile = _sLogPath + DateTime.Now.ToString("yyyyMMdd") + "_Gomaji_API.txt";
        ...
        {
            string vLogFile = _sErrLogPath + DateTime.Now.ToString("yyyyMMdd") + "_Gomaji_API.err";
            log = DateTime.Now.ToShortDateString() + " " + DateTime.Now.TimeOfDay + " " + log;
            if (_IsWriteErrLog)
            {
                if (File.Exists(vLogFile))
